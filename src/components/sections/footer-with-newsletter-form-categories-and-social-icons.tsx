...
export function FooterLink({ href, className, ...props }: { href: string } & Omit<ComponentProps<typeof Link>, 'to' | 'href'>) {
  return (
    <li className={clsx('text-mist-700 dark:text-mist-400', className)}>
      <Link to={href} className="hover:text-mist-950 dark:hover:text-white" {...props} />
    </li>
  )
}
